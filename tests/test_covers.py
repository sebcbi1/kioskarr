...
def test_unsupported_extension_returns_none(tmp_path):
    ...
    assert get_or_generate_cover(_issue(epub_path)) is None


def test_cbr_is_unsupported(tmp_path):
    cbr_path = tmp_path / "issue.cbr"
    cbr_path.write_bytes(b"fake rar content")

    assert get_or_generate_cover(_issue(cbr_path)) is None
